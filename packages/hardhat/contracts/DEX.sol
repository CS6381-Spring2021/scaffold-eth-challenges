...
contract DEX {
    ...
    // The total liquidity of the contract
    uint256 public totalLiquidity;

    // The liquidity of individual addresses
    mapping (address => uint256) public liquidity;

    /**
     * @notice Emitted when ethToToken() swap transacted
     */
    event EthToTokenSwap(address addr , string message , uint256 value, uint256 output);

    /**
     * @notice Emitted when tokenToEth() swap transacted
     */
    event TokenToEthSwap(address sender, string message, uint256 output, uint256 tokenInput);
